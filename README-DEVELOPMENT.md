...
`terraform-provider-crowdstrike`.  Substitute
...
* `provider/` contains the Go code used to create the provider as well as
  ...
  * `provider/cmd/pulumi-tfgen-crowdstrike`
...
1. In `provider/go.mod`, add a reference to the upstream Terraform provider in the `require` section, e.g.
    ...
    ```go
    github.com/foo/terraform-provider-crowdstrike v0.4.0
    ```

1. In `provider/resources.go`, ensure the reference in the `import` section uses the correct Go module path, e.g.:

    ```go
    github.com/foo/terraform-provider-crowdstrike/crowdstrike
    ```
...
1. Create the schema by running the following command:
    ...
    ```text
    warning: resource crowdstrike_something not found in provider map; skipping
    warning: resource crowdstrike_something_else not found in provider map; skipping
    warning: data source crowdstrike_something not found in provider map; skipping
    warning: data source crowdstrike_something_else not found in provider map; skipping
    ```
...
1. **Add resource mappings:** For each resource in the provider, add an entry in
   the `Resources` property of the `tfbridge.ProviderInfo`, e.g.:
    ...
    "crowdstrike_something":      {Tok: tfbridge.MakeResource(mainPkg, mainMod, "Something")},
    "crowdstrike_something_else": {Tok: tfbridge.MakeResource(mainPkg, mainMod, "SomethingElse")},
    ```
...
1. **Add data source mappings:** For each data source in the provider, add an entry in the `DataSources` property of the `tfbridge.ProviderInfo`, e.g.:
    ...
    "crowdstrike_something":      {Tok: tfbridge.MakeDataSource(mainPkg, mainMod, "getSomething")},
    "crowdstrike_something_else": {Tok: tfbridge.MakeDataSource(mainPkg, mainMod, "getSomethingElse")},
    ```
...
1. **Add provider configuration overrides (not typically needed):** Pulumi's
   ...
   `crowdstrike_API_KEY`.  Examples of common
   uses cases:
    ...
    "apikey": {
        Default: &tfbridge.DefaultInfo{
            EnvVars: []string{"crowdstrike_API_KEY"},
        },
    ```
...
1. Copy the `pulumi-resource-crowdstrike` binary
   generated by `make provider` and place it in your `$PATH` (`$GOPATH/bin` is a
   convenient choice), e.g.:

    ```bash
    cp bin/pulumi-resource-crowdstrike $GOPATH/bin
    ```
...
1. Python:
    ...
    pip install pulumi_crowdstrike
    ```
